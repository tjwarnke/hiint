...
func lectern(event, number):
	if number == 1 and lec1:
		text_box.text = "A book sits on the lectern, would you like to take it?"
		player.pick_up(book_on_lectern1)
	if number == 2 and lec2:
		text_box.text = "A book sits on the lectern, would you like to take it?"
	...
	if dialogue_active:
		if event.is_action_pressed("Option1"):
			...
			give_dialogue("\"The Kusnetzov family rose to power in the early 1700s. \nIn 1845, I became the first to move to america, then my younger siblings following soon after\"")
